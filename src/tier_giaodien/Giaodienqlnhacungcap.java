...
public class Giaodienqlnhacungcap extends JFrame {

	private JPanel contentPane;
	private JLabel lblNewLabel;
	private JTable table;
	private tablenccmodel thuocmodel;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Giaodienqlnhacungcap frame = new Giaodienqlnhacungcap();
	...
	public Giaodienqlnhacungcap() {
		...
		panel_3.setBorder(
				new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Chi ti\u1EBFt Nh\u00E0 cung c\u1EA5p",
						TitledBorder.LEADING, TitledBorder.TOP, null, new Color(128, 128, 128)));
		panel_3.setBounds(15, 27, 292, 249);
		panel_2.add(panel_3);
		panel_3.setLayout(null);

		JLabel lblTn = new JLabel("Tên ");
		lblTn.setBounds(15, 75, 21, 14);
		panel_3.add(lblTn);
		lblTn.setFont(new Font("Tahoma", Font.PLAIN, 11));

		JLabel lblNewLabel_1 = new JLabel("Mã ");
		lblNewLabel_1.setBounds(15, 16, 21, 20);
		panel_3.add(lblNewLabel_1);
		lblNewLabel_1.setFont(new Font("Tahoma", Font.PLAIN, 11));

		JLabel lblHnSDng = new JLabel("Địa chỉ");
		lblHnSDng.setBounds(15, 137, 69, 20);
		...
		textField_3.setBounds(15, 105, 262, 26);
		panel_3.add(textField_3);

		textField_4 = new JTextField();
		textField_4.setFont(new Font("Tahoma", Font.PLAIN, 11));
		textField_4.setColumns(10);
		textField_4.setBounds(15, 172, 262, 26);
		panel_3.add(textField_4);

		textField = new JTextField();
		textField.setFont(new Font("Tahoma", Font.PLAIN, 11));
		textField.setColumns(10);
		textField.setBounds(15, 44, 262, 26);
		...
		btnTmKim.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Nhacungcap_DAO tdao = new Nhacungcap_DAO();
				ArrayList<Nhacungcap> ds = tdao.Timkiem(id);
				ArrayList<Nhacungcap> ds1 = tdao.getdatafromnhacungcap();

				if (ds.size() < 0)
					lblNewLabel_2.setText("Không tìm thấy");
				else {
					thuocmodel = new tablenccmodel(ds);
	...
	public void fillForm(int row) {
		if (row != -1) {
			...
			ArrayList<Nhacungcap> ds = d.Timkiem(ma);

			if (ds != null) {
				Nhacungcap t = ds.get(0);
				textField.setText(t.getMa());

				textField_3.setText(t.getTen());

				textField_4.setText(t.getDiachi());
	...
	public Nhacungcap getdatafrominput() {
		String ma = textField.getText().trim();
		String ten = textField_3.getText().trim();
		String diachi = textField_4.getText().trim();

		if (!ma.matches("NCC[0-9]{4}")) {

			lblNewLabel_2.setText("Mã Nhà cung cấp phải bắt đầu bằng kí tự NCC theo sau là 5 chữ số");
		...
		if (!ten.matches(
			...
			lblNewLabel_2.setText("Tên gồm kí số hoặc'cách nhau bởi khoảng trắng");
			textField_3.requestFocus();
			return null;

		}
		if (!diachi.matches(
				"([ẮẰẲẴẶĂẤẦẨẪẬÂÁÀÃẢẠĐẾỀỂỄỆÊÉÈẺẼẸÍÌỈĨỊỐỒỔỖỘÔỚỜỞỠỢƠÓÒÕỎỌỨỪỬỮỰƯÚÙỦŨỤÝỲỶỸỴắằẳẵặăấầẩẫậâáàãảạđếềểễệêéèẻẽẹíìỉĩịốồổỗộôớờởỡợơóòõỏọứừửữựưúùủũụýỳỷỹỵa-zA-Z0-9',.-_]+[ ]?)+")) {
			lblNewLabel_2.setText("Địa chỉ gồm kí số hoặc',._-cách nhau bởi khoảng trắng");
			textField_4.requestFocus();
			return null;

		}
		System.out.println(new Nhacungcap(ma, ten, diachi));
